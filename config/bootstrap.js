...
module.exports.bootstrap = async function () {
  sails.config.chatUsuarios = {}
  sails.config.pendientesOffline = {};
  // Import dependencies
  var path = require("path");
  require("./passport");
  //sails.log.info("Inicializando datos base del sistema...");

  try {
    /**
     * 1️⃣ Crear rol SuperAdministrador si no existe
     */
    let superRole = await Role.findOne({ nombre: "SuperAdministrador" });
    ...
    if (!superRole) {
      ...
      //sails.log.info("Rol SuperAdministrador creado");
    }

    /**
     * 2️⃣ Crear permiso global si no existe
     */
    let permisoGlobal = await Permission.findOne({ nombre: "ALL_ACCESS" });

    if (!permisoGlobal) {
      permisoGlobal = await Permission.create({
        nombre: "ALL_ACCESS",
        ruta: "*",
        metodo: "*",
      },{
        nombre:"SinPermiso",
        ruta:"/no-authorization",
        method:"get"
      }).fetch();

      //sails.log.info("Permiso global creado");
    }

    /**
     * 3️⃣ Asignar permiso al rol
     */
    let permisosRol = await Role.findOne({ id: superRole.id }).populate(
      "permisos",
    );
    //sails.log.info("rol:")
    //sails.log.info(permisosRol);

    let tienePermiso = permisosRol.permisos.some(
      (p) => p.nombre === "ALL_ACCESS",
    );

    if (!tienePermiso) {
      await Role.addToCollection(superRole.id, "permisos").members([
        permisoGlobal.id,
      ]);

      sails.log.info("Permiso global asignado al rol");
    }

    /**
     * 4️⃣ Crear usuario administrador si no existe
     */

    let admin = await User.findOne({ email: "administrador@exi.mx" });

    if (!admin) {
      let passwordHash = await bcrypt.hash("Exi01+", 10);

      admin = await User.create({
        nombre: "Administrador",
        email: "administrador@exi.mx",
        password: passwordHash,
        roles: superRole.id,
      },{
        nombre: "Amadeo",
        email: "amadeo.espinosa@exi.mx",
        password: passwordHash,
        roles: superRole.id,
      }).fetch();

      sails.log.info("Usuario administrador creado");
    }

    sails.log.info("Datos iniciales verificados correctamente");
  } catch (err) {
    sails.log.error("Error en bootstrap:", err);
  }
